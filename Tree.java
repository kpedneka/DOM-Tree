		...
		while (sc.hasNext()){
			...
			if (temp.charAt(0) != '<'){
				...
				if (temp.charAt(1) == '/'){
					...
					//new tag. This gives problems. Can't find a condition to decide where to add new tag
					temp = temp.substring(1,temp.length()-1);
					if (ptr == prev){ // just popped something
						ptr.sibling = new TagNode (temp, null, null);
						prev = ptr;
						ptr = ptr.sibling;
						stk.push(ptr);
					} else {
						if (prev.firstChild != null){ //adding a sibling
							ptr.sibling = new TagNode (temp, null, null);
							prev = ptr;
							ptr = ptr.sibling;
							stk.push(ptr);
						} else { //adding a firstChild
							ptr.firstChild = new TagNode (temp, null, null);
							prev = ptr;
							ptr = ptr.firstChild;
							stk.push(ptr);
